...
// 更新日: 2024-07-08
// 更新者: Yuya Wakita
// コメント: writeCSVenergyConsumption()を追加
////////////////////////////////////////////////////////////////////////////////////
...
// 説明: EnergyEstimator入力用のCSVを生成する
// 引数: filename CSVファイル名
...
// 説明: EnergyEstimatorの計算結果をCSVで出力する
// 引数: filename CSVファイル名
//      time 時間[sec]
//      speed 車速[km/h]
//      gradient 勾配[rad]
//      energyConsumption 電力消費瞬時値[kw]
...
void writeCSVenergyConsumption(const std::string& filename,
    ...
    // ヘッダー行を書き込む
    file << "time_sec,speed_kmph,gradient_rad,energy_consumption_kw,energy_consumption_kWh\n";

    // データ行を書き込む
    for (size_t i = 0; i < time.size(); ++i) {
        file << time[i] << "," << speed[i] << "," << gradient[i] << "," << energyConsumption[i] << "," << energyConsumptionKwh[i] << "\n";
    }
